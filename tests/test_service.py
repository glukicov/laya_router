...
def test_triage_returns_every_question_in_the_schema() -> None:
    with TestClient(create_app(backend_builder=FakeBackend)) as client:
        body = client.post("/triage", json={"message": "hello"}).json()
...
def test_invalid_requests_are_rejected_before_the_model_runs() -> None:
    ...
    with TestClient(create_app(backend_builder=lambda: backend)) as client:
        empty = client.post("/triage", json={"message": ""})
        extra = client.post("/triage", json={"message": "valid", "unexpected": True})
...
def test_concurrent_requests_all_get_answered() -> None:
    ...
    with (
        ...
        responses = list(pool.map(lambda m: client.post("/triage", json={"message": m}), ["a", "b", "c", "d"]))
