...
BACKEND_LABEL = {"laya": "Laya 421M, local", "openai": "OpenAI classifier"}
...
def accuracy(summaries: dict[str, dict[str, Any]]) -> Path:
    ...
    ax.set_title("Same questions, same messages", loc="left")
...
def deferral(summaries: dict[str, dict[str, Any]]) -> Path:
    ...
    ax.set_xlabel("decisions answered automatically (%), most confident first")
    ax.set_ylabel("accuracy of what was answered")
    ax.set_title("Automate the confident ones, escalate the rest", loc="left")
...
def cost_latency(summaries: dict[str, dict[str, Any]]) -> Path:
    ...
    left.set_title("Time to one decision", loc="left")
    ...
    right.set_ylabel("USD per 1,000 messages")
    right.set_ylim(0, max([*costs, 0.01]) * 1.3)
    right.set_title("Cost of one thousand decisions", loc="left")
...
def difficulty(summaries: dict[str, dict[str, Any]]) -> Path:
    ...
    ax.set_title("Where each backend struggles", loc="left")
    ax.legend(loc="lower right")
...
def render_all(metrics_path: Path) -> list[Path]:
    """Render every figure from a saved metrics file."""
